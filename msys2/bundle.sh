...
# 1. Main executable placed directly in the root (flattened structure, no bin/ subfolder)
cp "${MINGW_PREFIX}/bin/gtkhash.exe" "$DEST/"

# 2. Patch PE subsystem to "windows" (GUI) – suppresses the background console window
if command -v objcopy >/dev/null 2>&1; then
    objcopy --subsystem windows "$DEST/gtkhash.exe" || true
fi

# 3. Copy GTK loaders and modules into their required subdirectories (these are NOT flattened)
...
# 4. Recursively resolve and copy all dynamically linked DLLs to the root (flat layout)
resolve_deps() {
    local added=1
    while [ $added -eq 1 ]; do
        added=0
        ...
        for f in "${files[@]}"; do
            ...
            for d in "${deps[@]}"; do
                if [ -n "$d" ] && [ "$d" != "not" ]; then
                    ...
                    if [ -f "$posix_path" ] && [ ! -f "$DEST/$basename_d" ]; then
                        cp "$posix_path" "$DEST/"
                        added=1
                    fi
                fi
